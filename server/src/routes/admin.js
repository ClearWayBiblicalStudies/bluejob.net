...
router.patch('/evidence/:id', async (req, res) => {
  ...
  const { rows } = await pool.query('UPDATE evidence SET status=$1, verified_at=now(), verified_by=$2 WHERE id=$3 RETURNING id,user_id,status', [req.body.status, req.user.sub, req.params.id]);
