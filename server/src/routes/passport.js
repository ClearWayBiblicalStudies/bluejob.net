...
router.post('/evidence', upload.single('file'), async (req,res) => { if (!req.file) return res.status(400).json({ error:'A document is required' }); const scan=await scanFile(req.file.path); const { rows } = await pool.query('INSERT INTO evidence (user_id,type,original_name,stored_name,status,sha256) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id,type,original_name AS file,status,created_at',[req.user.sub,req.body.type,req.file.originalname,req.file.filename,scan.status,scan.sha256]); if(scan.status==='Rejected') return res.status(422).json({error:'File failed malware scan'}); res.status(201).json({ evidence: rows[0] }); });
