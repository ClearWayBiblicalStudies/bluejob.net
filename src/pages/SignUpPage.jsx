  ...
  return (
    <div className="auth-page">
      ...
      <div className="auth-card white-card">
        ...
        <form onSubmit={async (e) => { e.preventDefault(); const form = new FormData(e.currentTarget); try { const result = await api.signup({ name: form.get('name'), email: form.get('email'), password: form.get('password') }); setToken(result.token); navigate('/choose-path'); } catch (err) { e.currentTarget.querySelector('[role=alert]').textContent = err.message; } }}>
