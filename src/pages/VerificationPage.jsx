...
import { readEvidence, saveEvidence } from '../lib/passport';
...
export default function VerificationPage() {
  ...
  const submit = (event) => { event.preventDefault(); if (!file) return; const next = [...evidence, { id: crypto.randomUUID(), type, file, status: 'Pending Verification' }]; setEvidence(next); saveEvidence(next); setFile(''); event.target.reset(); };
  return <AppShell role="worker"><div className="page-heading"><div><span className="kicker">PRIVATE DOCUMENTS</span><h1>Verification Center</h1><p>Submit proof privately. Verified results appear on your public Work Passport.</p></div><Link to="/app/passport" className="back-link">View public Passport →</Link></div>
    ...
    <div className="verification-layout"><section className="card evidence-upload"><div className="section-heading"><div><h2>Submit evidence</h2><p>Every submission starts as Pending Verification.</p></div><ShieldCheck className="green-text"/></div><form onSubmit={submit}><label>Evidence type<select value={type} onChange={(e) => setType(e.target.value)}>{types.map((item) => <option key={item}>{item}</option>)}</select></label><label>Choose document<input type="file" onChange={(e) => setFile(e.target.files[0]?.name || '')} required /></label><button className="button button-primary"><Plus size={15}/> Submit for review</button></form></section>
