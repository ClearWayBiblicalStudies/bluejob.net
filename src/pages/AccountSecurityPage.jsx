...
export default function AccountSecurityPage() {
  ...
  return <AppShell><div className="page-heading"><div><span className="kicker">ACCOUNT SECURITY</span><h1>Identity and access</h1><p>Verify your contact methods and protect your account with MFA.</p></div></div>
    ...
    <section className="card"><h2>Founding Membership</h2><p>Unlock private beta access for $19.99/month.</p><button className="button button-outline" onClick={async () => { try { const { url } = await api.membershipCheckout(); window.location.assign(url); } catch (error) { setMessage(error.message); } }}>Open secure checkout</button></section>
